...
const AdminSidebar = ({ isOpen, onClose }) => {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside className={`
        fixed top-0 left-0 z-50 h-screen w-80 bg-slate-900 text-white transform transition-all duration-300 ease-in-out flex flex-col
        lg:translate-x-0 lg:static lg:z-auto
        ${isOpen ? 'translate-x-0' : '-translate-x-full'}
      `}>
        {/* Header with Logo */}
        <div className="flex-shrink-0 flex items-center justify-between h-20 px-6 border-b border-slate-800">
